...
/obj/item/bodypart/taur/get_limb_icon(dropped, hideaux = FALSE)
	...
	if(statue_color || bodypart_owner?.IsPetrified())
		petrification_debug("taur_get_limb_icon start: [petrification_debug_bodypart_summary(src)] dropped=[dropped] owner=[petrification_debug_value(bodypart_owner)] owner_petrified=[bodypart_owner?.IsPetrified()] statue_color=[petrification_debug_value(statue_color)] taur_color=[petrification_debug_value(taur_color)] matrix_len=[petrification_debug_len(petrified_color_matrix)]")
	// OV Edit End
