...
/mob/living/carbon/human/send_speech(message, message_range = 6, atom/movable/source = src, bubble_type = bubble_icon, list/spans, datum/language/message_language=null, message_mode, original_message)
	. = ..()
	var/atom/movable/speech_source = source || src
	if(message_mode != MODE_WHISPER)
		send_voice(message, null, speech_source)
	else	//OV ADD
...
/mob/living/carbon/human/proc/send_voice(message, skip_thingy, atom/movable/sound_source = src)
	if(!message || !length(message))
		return
	if(dna.species)
		dna.species.send_voice(src, sound_source)

/datum/species/proc/send_voice(mob/living/carbon/human/H, atom/movable/sound_source = H)
	playsound(get_turf(sound_source), 'sound/misc/talk.ogg', 100, FALSE, -1)
